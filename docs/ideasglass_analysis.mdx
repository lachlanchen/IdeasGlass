...
- The Flutter client already targets Android/iOS/Web/Desktop, talks to the wearable over BLE (`flutter_blue_plus` etc.), then syncs up to a FastAPI service that leans on Firebase, Redis, Pinecone, OpenAI/Deepgram, and Modal serverless workers.
...
- Deployment story mixes Modal serverless (`modal_app`), Firebase Admin SDK, and GCP secrets.
...
3. **Sync:** App uploads transcripts/media to the FastAPI backend via HTTPS, which then pushes data into Firebase/Pinecone/OpenAI pipelines.
