...
This guide documents the exact steps we used to relay Arduino data to a public HTTPS endpoint (`https://ideas.lazying.art`) and display it in a PWA that installs on Android/iOS/Desktop. The stack consists of:
...
2. **Install requirements (already done but harmless to repeat):**
   ...
   ```
3. **Launch uvicorn on an unused port (8765 chosen to avoid collisions):**
   ```bash
...
4. **Ngrok tunnel (maps the public domain to your local port):**
   ```bash
...
5. **Verify backend health:** `curl https://ideas.lazying.art/healthz` should return `{"status":"ok","messages":...}`.
6. **Open the PWA dashboard** at `https://ideas.lazying.art/`:
   - You should see **Backend Online / WebSocket Connected** states.
   - Use the browser’s “Add to Home Screen” option (or the built-in “Add” button) to install it on Android/iOS.
...
2. Open `IdeaGlass/firmware/ideasglass_arduino/IdeasGlassNgrokClient/IdeasGlassNgrokClient.ino` in Arduino IDE.
   ...
   - The LetsEncrypt chain for `ideas.lazying.art` is embedded so `WiFiClientSecure` can validate Ngrok’s cert.
3. Upload to the XIAO ESP32S3. Serial monitor will show Wi-Fi status and repeated `POST /api/v1/messages` responses.
4. Each payload appears instantly in the PWA feed (and via the `/api/v1/messages` REST endpoint).
...
- **Manual test via curl:**
  ...
  curl -X POST https://ideas.lazying.art/api/v1/messages \
    ...
    -d '{"device_id":"curl-test","message":"hello from CLI"}'
  ```
