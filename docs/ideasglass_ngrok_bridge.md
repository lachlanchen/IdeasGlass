...
- **Backend** — runs WebRTC VAD (`webrtcvad`), normalizes each chunk toward `IDEASGLASS_GAIN_TARGET` (default 0.032 RMS, capped by `IDEASGLASS_GAIN_MAX`), streams every chunk to disk immediately, and only finalizes a WAV segment once ~60 s of audio plus trailing silence have accumulated. Raw chunks still land in `ig_audio_chunks`, while referenced WAVs live in `ig_audio_segments`.
