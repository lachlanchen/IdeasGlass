...
- OpenAI Whisper-powered live transcripts (config via `IDEASGLASS_WHISPER_*`) streamed under the waveform with updates every few seconds; final clips are cached in history
...
- **Transcription** – set `IDEASGLASS_TRANSCRIBE=1` (default) with `openai-whisper` installed to enable streaming transcripts. Use `IDEASGLASS_WHISPER_MODEL` (e.g. `base`, `small`, `turbo`), `IDEASGLASS_WHISPER_DEVICE` (`cuda`/`cpu`), and `IDEASGLASS_TRANSCRIPT_INTERVAL_MS` to tune performance; disable via `IDEASGLASS_TRANSCRIBE=0` if you need to pause it.
