...
- **Transcription** – set `IDEASGLASS_TRANSCRIBE=1` (default) with `whisperx` installed to enable WhisperX large-v2 on `IDEASGLASS_WHISPERX_DEVICE` (`cuda`/`cpu`). Provide `IDEASGLASS_HF_TOKEN` or `HUGGINGFACE_TOKEN` for diarization; disable via `IDEASGLASS_TRANSCRIBE=0` if GPU resources are tight.
