...
// Device name buffer, populated at runtime using a per-device identifier.
// Format: "LightMind-XXXXXXXX" (base prefix + 8 hex digits).
...
static struct bt_data bt_ad[] = {
    ...
    BT_DATA(BT_DATA_UUID128_ALL, audio_service_uuid.val, sizeof(audio_service_uuid.val)),
    {
        ...
        .data_len = 0, // set at runtime by init_device_name()
...
static void init_device_name(void)
{
    /* Use the hardware device ID as a stable per-unit identifier. */
#ifdef NRF_FICR
    uint64_t raw_id = ((uint64_t)NRF_FICR->DEVICEID[1] << 32) | NRF_FICR->DEVICEID[0];
    uint32_t tag = (uint32_t)(raw_id & 0xFFFFFFFFu);
#else
    /* Fallback: use a fixed tag if FICR is not available on this build. */
    uint32_t tag = 0x12345678u;
#endif

    /* Format as LightMind-XXXXXXXX where XXXXXXXX are hex digits. */
    int written = snprintf(device_name, sizeof(device_name), "LightMind-%08X", tag);
    if (written < 0) {
        /* Leave name empty on failure; advertising will still run. */
        device_name[0] = '\0';
        bt_ad[2].data_len = 0;
        return;
    }
    ...
    bt_ad[2].data_len = (uint8_t)len;

#if defined(CONFIG_BT_DEVICE_NAME_DYNAMIC)
    int err = bt_set_name(device_name);
    if (err) {
        LOG_WRN("Failed to set GAP device name (%d)", err);
    }
#endif

    LOG_INF("Using BLE name: %s", device_name);
}
...
int transport_start()
{
    ...
    /* Prepare per-device BLE name before advertising (uses hardware ID). */
    init_device_name();
